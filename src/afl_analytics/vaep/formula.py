...
def offensive_value(
    actions: DataFrame[ARPADLSchema], scores: Series[float], concedes: Series[float]
) -> Series[float]:
    ...
    prevgoal_idx = (_prev(actions.action_type).isin(["shot"])) & (
        _prev(actions.result) == "goal"
    )
...
def value(
    actions: DataFrame[ARPADLSchema], Pscores: Series[float], Pconcedes: Series[float]
) -> pd.DataFrame:
